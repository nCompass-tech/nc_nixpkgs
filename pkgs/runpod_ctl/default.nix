...
pkgs.stdenv.mkDerivation rec {
  ...
  arch = if pkgs.stdenv.hostPlatform.system == "aarch64-darwin" then "arm64" else "amd64";
  ...
  src = pkgs.fetchurl {
    url = "https://github.com/runpod/runpodctl/releases/download/v${version}/runpodctl-${os}-${arch}";
    sha256 = "${sha256}";
  };
